  ...
  const onCredentialsSubmit = async (values: LoginFormValues) => {
    ...
    try {
      ...
      if (result?.error) {
        ...
        toast.error("Login Failed", { description: "An error occurred while trying to sign in." });
    } finally {
     ...
     <div className="flex justify-center items-center min-h-screen bg-muted/40 px-4 py-12">
      ...
      <Card className="w-full max-w-md shadow-md">
        ...
        <CardContent className="grid gap-6">
          ...
          {/* Credentials Form */}
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onCredentialsSubmit)} className="grid gap-4">
              {/* Email Field */}
              <FormField
              ...
              <Button type="submit" className="w-full" disabled={isLoading}>
                {isLoadingCredentials && <Icons.spinner className="mr-2 h-4 w-4 animate-spin" />}
                Sign In
              </Button>
            </form>
          </Form>
